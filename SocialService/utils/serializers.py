...
class PermissionModelSerializer(RemoveDeletedModelSerializer):
    ...
    def filter_using_permissions(self, data, action, app_label, action_alt=None):
        ...
        contenttype = ContentType.objects.filter(model=model, app_label=app_label).first()
        ...
        permission = contenttype.permission_set.filter(codename__startswith__in=[action, action_alt]).exist()
        if permission:
            return data, errors
        ...
        for field, value in data.items():
            ...
            codename = f"{model}|{action}|{field}".lower()
