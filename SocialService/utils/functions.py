...
from utils.choices import ChatTypeChoices
import random
import datetime
...
def check_is_vulgar(message):
    """This uses AI to check the message vulgarity"""
    return True
...
def get_channel_name(user, friend, purpose, nickname=""):
    if purpose==ChatTypeChoices.ONE_ON_ONE:
        if user.id < friend.id:
            return get_channel_name(friend, user, purpose)
        purpose = ChatTypeChoices._value2member_map_[purpose].name
        nickname = f"{user.id}<==chat==>{friend.id}<==purpose==>{purpose}"
    elif purpose==ChatTypeChoices.GROUP:
        purpose = ChatTypeChoices._value2member_map_[purpose].name
        nickname = f"{user.id}<==chat==>{nickname}<==purpose==>{purpose}"        
    payload = json.dumps({
        "name": nickname,
    }).encode()
    return hashlib.sha512(payload).hexdigest(), nickname
